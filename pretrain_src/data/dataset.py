...
class ReverieTextPathData(object):
    def __init__(
        ...
        self.obj_ft_file = obj_ft_file  # './datasets/REVERIE/features/obj.avg.top3.min80_vit_base_patch16_224_imagenet.hdf5'

        self.image_feat_size = image_feat_size  # 2048
        self.image_prob_size = image_prob_size  # 1000
        self.angle_feat_size = angle_feat_size  # 4
        self.obj_feat_size = obj_feat_size  #   768
        self.obj_prob_size = obj_prob_size  # 1000
        ...
        self.max_objects = max_objects  # 20
        ...
        self.scanvp_cands = json.load(open(scanvp_cands_file))  # scanvp_cands_file = './datasets/R2R/annotations/scanvp_candview_relangles.json'

        self.graphs, self.shortest_distances, self.shortest_paths = load_nav_graphs(connectivity_dir)  # connectivity_dir = './datasets/R2R/connectivity'，获得图、最短距离、最短路径
        self.all_point_rel_angles = [get_view_rel_angles(baseViewId=i) for i in range(36)]  # 得到视角列表，每个元素尺寸为 36*2
        self.all_point_angle_fts = [get_angle_fts(x[:, 0], x[:, 1], self.angle_feat_size) for x in self.all_point_rel_angles]  # 得到视角的角度特征列表，每个元素 36*4

        self.data = []
        # 读取注释文件
        for anno_file in anno_files:  # './datasets/REVERIE/annotations/pretrain/REVERIE_train_enc.jsonl'，'./datasets/REVERIE/annotations/pretrain/REVERIE_speaker_aug_enc.jsonl'
    ...
    def get_input(
        ...
        item = self.data[idx]  # 得到一条数据
        scan = item['scan']  # 得到一个建筑场景
        start_vp = item['path'][0]  # 得到路径起始点图像名，即起始点位置
        start_heading = item.get('heading', 0)  # 得到起始点的朝向
        pos_vps = item['pos_vps']  # 
        gt_path = item['path']  # 得到gt路径

        if end_vp is None:
            if end_vp_type == 'pos':
                end_vp = pos_vps[np.random.randint(len(pos_vps))]  # 路径终止点图像名
        ...
        gt_path = self.shortest_paths[scan][start_vp][end_vp]  # 开始点到终点的最短路径
        cur_heading, cur_elevation = self.get_cur_angle(scan, gt_path, start_heading)  # 得到当前视角的朝向和仰角
...
# 获得路径的视角特征、物体特征、物体属性
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            view_fts, obj_img_fts, obj_attrs = self.get_scanvp_feature(scan, vp)  # 获得视角特征、物体特征、物体属性
