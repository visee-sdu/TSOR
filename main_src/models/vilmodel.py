...
class CrossAttention(nn.Module):
    ...
    def forward(self, hidden_states, n_embeds):
        """
        hidden_states: (N, L_{hidden}, D)
        attention_mask: (N, H, L_{hidden}, L_{hidden})
        """
        mixed_query_layer = self.query(n_embeds)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        ...
        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        # print("attenion_probs: ", attention_probs)
...
class LanguageEncoder(nn.Module):
    ...
    def forward(self, txt_embeds, txt_masks, n_masks, token_ids, token_indexs_len):
        ...
        n_lengths = [sum(n_mask) for n_mask in n_masks]  
        n_embeds = torch.zeros((len(n_masks), max(n_lengths), txt_embeds.shape[-1]), dtype=torch.float32)
        for i, (n_mask, txt_embed) in enumerate(zip(n_masks, txt_embeds)): 
            n_embeds[i,0:n_lengths[i]] = txt_embed[n_mask]
            
        token_embeds = torch.zeros(len(txt_embeds), max(token_indexs_len), txt_embeds.size(-1)).cuda()
        
        if max(token_indexs_len) > 0:
            for i, (embed, token_id, token_index_len) in enumerate(zip(txt_embeds, token_ids, token_indexs_len)):
                token_embeds[i, 0:token_index_len] = torch.index_select(embed, 0, token_id[0:token_index_len])
            
        return txt_embeds, n_embeds, token_embeds
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.vison_attention = CrossAttention(config)
        ...
        self.w = nn.Parameter(torch.Tensor([-2.3, -2.3, -0.22]))
        with open('./matrix/matrix.json', 'r') as f:
            self.matrix = nn.Parameter(torch.tensor(json.load(f)))
    ...
    def forward_text(self, txt_ids, txt_masks, n_masks, token_ids, token_indexs_len):
        txt_token_type_ids = torch.zeros_like(txt_ids)
        txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
        txt_embeds, n_embeds, token_embeds= self.lang_encoder(txt_embeds, txt_masks, n_masks, token_ids, token_indexs_len)
        return txt_embeds, n_embeds, token_embeds

    def forward_panorama_per_step(
        self, view_img_fts, obj_img_fts, loc_fts, nav_types, view_lens, obj_lens, n_embeds, obj_indexs, token_indexs, token_indexs_len, token_embeds
    ):
        ...
        if has_obj:
            ...
            n_embeds = self.vison_attention(n_embeds, obj_img_embeds) 
            
            
            if max(token_indexs_len) > 0:
                sim = torch.zeros(len(obj_lens), max(obj_lens), max(token_indexs_len)).cuda()
                for i, (obj_index, token_index) in enumerate(zip(obj_indexs, token_indexs)):
                    if len(obj_index)>0 and len(token_index)>0:
                        matrix1 = torch.index_select(self.matrix, 0 , obj_index[0:obj_lens[i]])
                        matrix2 = torch.index_select(matrix1, 1, token_index[0:token_indexs_len[i]])
                        sim[i, 0:obj_lens[i], 0:token_indexs_len[i]] = matrix2
                value = torch.bmm(sim, token_embeds)
                
            w1 = torch.exp(self.w[0]) / torch.sum(torch.exp(self.w))
            w2 = torch.exp(self.w[1]) / torch.sum(torch.exp(self.w))
            w3 = torch.exp(self.w[2]) / torch.sum(torch.exp(self.w))
            obj_img_embeds =  w1*n_embeds + w2*value + w3*obj_img_embeds
            ...
            n_embeds = self.vison_attention(n_embeds, view_img_embeds)
            w1 = torch.exp(self.w[0]) / torch.sum(torch.exp(self.w))
            w2 = torch.exp(self.w[1]) / torch.sum(torch.exp(self.w))
            view_img_embeds = w1*n_embeds + w2*view_img_embeds
    ...
    '''
    gmap_step_ids:16*13
    gmap_img_embeds: 16*13*768
    gmap_img_masks:16*13
    gmap_pair_dists:16*13*13
    gmap_pos_fts:16*13*7
    gmap_visited_masks:16*13
    gmap_vpids:16
    txt_embeds:16*36*768
    txt_masks:16*36
    vp_cand_vpids:16
    vp_img_embeds:16*57*768
    vp_masks:16*57
    vp_nav_masks:16*57
    vp_obj_masks:16*57
    vp_pos_fts:16*57*14
    ...
    def forward_navigation_per_step(
        ...
        global_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))  # 16*10
        

        local_logits = self.local_sap_head(vp_embeds).squeeze(2) * (1 - fuse_weights)
        local_logits.masked_fill_(vp_nav_masks.logical_not(), -float('inf'))  # 16*51
        

        # fusion
        fused_logits = torch.clone(global_logits)
        fused_logits[:, 0] += local_logits[:, 0]   # stop,16*10
        for i in range(batch_size):
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            txt_embeds, n_embeds, token_embeds= self.forward_text(batch['txt_ids'], batch['txt_masks'], batch['n_masks'], batch['token_ids'], batch['token_indexs_len'])
            return txt_embeds, n_embeds, token_embeds  

        elif mode == 'panorama':
            pano_embeds, pano_masks= self.forward_panorama_per_step(
                batch['view_img_fts'], batch['obj_img_fts'], batch['loc_fts'],
                batch['nav_types'], batch['view_lens'], batch['obj_lens'], batch['n_embeds'], batch['obj_indexs'], batch['token_indexs'], batch['token_indexs_len'], batch['token_embeds']
            )
            return pano_embeds, pano_masks  
