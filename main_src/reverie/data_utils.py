...
class ObjectFeatureDB(object):
    ...
    # 得到scan_viewpoint下的所有物体的视觉特征，维度obj_feat_size,物体最多20个，不够的用0填充
    ...
    # 获得scan_viewpoint下的所有物体的视觉特征、角度特征、大小特征、物体id
...
# 返回一个列表[data]，每一条data数据包含instr_id，instruction，path_id，scan，objId，instr_encoding
def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512):
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits, tokenizer)):
        ...
        for j, instr in enumerate(item['instructions']):
            new_item = dict(item)
            if 'objId' in item:
                new_item['instr_id'] = '%s_%s_%d' % (str(item['path_id']), str(item['objId']), j)  # pathId_objId_j
            else:
