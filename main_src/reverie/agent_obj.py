...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _build_model(self):
        ...
        # buffer
        self.scanvp_cands = {}
        with open('./matrix/nouns_id.json', 'r') as f:
            self.nouns_id = json.load(f)
            
        with open('./matrix/obj_id2index.json', 'r') as f:
            self.old_obj_id2index = json.load(f)
            self.obj_id2index = {int(k): v for k, v in self.old_obj_id2index.items()}
        with open('./matrix/token_id2index.json', 'r') as f:
            self.old_token_id2index = json.load(f)
            self.token_id2index = {int(k): v for k, v in self.old_token_id2index.items()}
    ...
    def _language_variable(self, obs):
        ...
        n_mask = np.zeros((len(obs), max(seq_lengths)), dtype=np.bool_)  
        
        batch_token_indexs = []
        batch_token_indexs_len = []
        batch_token_ids = []
        ...
        for i, ob in enumerate(obs):
            ...
            n_tensor = [1 if digit in self.nouns_id else 0 for digit in ob['instr_encoding']]
            n_mask[i, :seq_lengths[i]] = n_tensor
            
            token_indexs = []
            token_ids = []
            for j,digit in enumerate(ob['instr_encoding']):
                if digit in self.token_id2index.keys():
                    token_ids.append(j)
                    token_indexs.append(self.token_id2index[digit])  
            batch_token_ids.append(torch.tensor(token_ids).long())  
            batch_token_indexs.append(torch.tensor(token_indexs).long())
            batch_token_indexs_len.append(len(token_indexs)) 

        seq_tensor = torch.from_numpy(seq_tensor).long().cuda()
        mask = torch.from_numpy(mask).cuda()
        n_mask = torch.from_numpy(n_mask).cuda()
        
        batch_token_ids = pad_tensors(batch_token_ids).cuda()
        batch_token_indexs = pad_tensors(batch_token_indexs).cuda()
        batch_token_indexs_len = torch.LongTensor(batch_token_indexs_len).cuda()
        
        return {
            'txt_ids': seq_tensor, 'txt_masks': mask, 'n_masks': n_mask, 'token_ids':batch_token_ids, 'token_indexs': batch_token_indexs, 'token_indexs_len': batch_token_indexs_len,
        }
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            obj_indexs = [self.obj_id2index[int(obj_id)] for obj_id in ob['obj_ids']]
            obj_indexs = torch.tensor(obj_indexs).long()
            batch_obj_indexs.append(obj_indexs)
        ...
        batch_obj_indexs = pad_tensors(batch_obj_indexs).cuda()
    ...
    def _teacher_action(self, obs, vpids, ended, visited_masks=None):       
        ...
        max_length = max([len(vpid) for vpid in vpids])
        b = np.zeros((len(obs), max_length), dtype=np.float32)
        for i, ob in enumerate(obs):
                    ...
                    for j, vpid in enumerate(vpids[i]):
                        if j > 0 and ((visited_masks is None) or (not visited_masks[i][j])):
                            ...
                            if dist < min_dist:
                                min_dist = dist
                                min_idx = j
                            if len(self.env.shortest_paths[scan][cur_vp][vpid]) > 1:
                                pos = self.env.shortest_paths[scan][cur_vp][vpid][-2]
                                min_path = self.env.shortest_distances[scan][cur_vp][pos]
                                b[i][j] = min_path
                    a[i] = min_idx
    ...
    def _teacher_step(self, obs, vpids, ended, visited_masks=None):
        """
        Extract teacher actions into variable.
        :param obs: The observation.
        :param ended: Whether the action seq is ended
        ...
        for i, ob in enumerate(obs):
            if ended[i]:                                            # Just ignore this index
                ...
                if ob['viewpoint'] == ob['gt_path'][-1]:
                    ...
                    scan = ob['scan']
                    cur_vp = ob['viewpoint']
                    min_idx, min_dist, min_step = self.args.ignoreid, float('inf'), float('inf')
                    for j, vpid in enumerate(vpids[i]):
                        if j > 0 and ((visited_masks is None) or (not visited_masks[i][j])):
                            step = len(self.env.shortest_paths[scan][vpid][ob['gt_path'][-1]]) \
                                    + len(self.env.shortest_paths[scan][cur_vp][vpid])
                            dist = self.env.shortest_distances[scan][vpid][ob['gt_path'][-1]] \
                                    + self.env.shortest_distances[scan][cur_vp][vpid]
                            if step < min_step:
                                min_dist = dist
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        txt_embeds, n_embeds, token_embeds= self.vln_bert('language', language_inputs)
        ...
        for t in range(self.args.max_action_len):
            ...
            pano_inputs['n_embeds'] = n_embeds.cuda()
            pano_inputs['token_indexs'] = language_inputs['token_indexs']
            pano_inputs['token_indexs_len'] = language_inputs['token_indexs_len']
            pano_inputs['token_embeds'] = token_embeds.cuda()
            ...
            visited_nodes = nav_inputs['gmap_visited_masks']
            
            for i, ob in enumerate(obs):
                scan = ob['scan']
                cur_vp = ob['viewpoint']
                for j, vpid in enumerate(nav_vpids[i]):
                        if j > 0 and ((visited_nodes is None) or (not visited_nodes[i][j])):
                            if len(self.env.shortest_paths[scan][cur_vp][vpid]) > 1:
                                nav_logits[i][j] = nav_logits[i][j]*2
            ...
            if train_ml is not None:
                # Supervised training
                nav_targets, b_targets = self._teacher_action(
                    obs, nav_vpids, ended, 
                    visited_masks=nav_inputs['gmap_visited_masks'] if self.args.fusion != 'local' else None
                )
                b_loss += torch.sum(torch.softmax(nav_logits, dim=-1)*b_targets)
                ...
                if self.args.fusion in ['avg', 'dynamic'] and self.args.loss_nav_3:
                    ...
                    local_nav_targets, _ = self._teacher_action(
        ...
        if train_ml is not None:
            ...
            b_loss = b_loss * train_ml / batch_size
            b_loss = b_loss*0.2
            self.loss += ml_loss
            self.loss += og_loss
            self.loss += b_loss
            ...
            self.logs['B_loss'].append(b_loss.item())
        return traj
