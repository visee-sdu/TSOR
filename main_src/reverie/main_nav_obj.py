...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    listner = agent_class(args, train_env, rank=rank)  # 初始化了一个GMapObjectNavAgent

    # resume file
    start_iter = 0
    if args.resume_file is not None:
        start_iter = listner.load(os.path.join(args.resume_file))  # 加载模型参数
        if default_gpu:
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            B_loss = sum(listner.logs['B_loss']) / max(len(listner.logs['B_loss']), 1)
            entropy = sum(listner.logs['entropy']) / total
            writer.add_scalar("loss/critic", critic_loss, idx)
            writer.add_scalar("policy_entropy", entropy, idx)
            writer.add_scalar("loss/OG_loss", OG_loss, idx)
            writer.add_scalar("loss/IL_loss", IL_loss, idx)
            writer.add_scalar("loss/B_loss", B_loss, idx)
            writer.add_scalar("total_actions", total, idx)
            writer.add_scalar("max_length", length, idx)
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, OG_loss %.4f, B_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, OG_loss, B_loss, policy_loss, critic_loss),
                record_file
            )
